...
const {repoNew,device,ok,done,wait,CFG,cfg,D}=require("./gh.js");

(async()=>{
  const repo=repoNew();
  console.log("【手机】记了 09-14 三顿 + 一条疫苗");
  const A=device("手机",repo,{ "catfeed.v3":D({
    upd:1000, marks:{"2026-09-14":{a0:1,a1:1,b0:1}},
    wa:{"2026-09-14":5.0}, wb:{}, notes:{"2026-09-14":[{t:"疫苗",c:"b",x:"三联第一针",at:1000}]}
  })}, CFG);
  ok("手机上脚本跑通", !A.threw, A.threw?A.threw.message:"");
  await wait(300);
  ok("两个文件都推上去了", !!(repo.files["settings.json"]&&repo.files["2026.json"]),
     Object.keys(repo.files).join(", "));

  console.log("\n【电脑】从没同步过，本地只有 09-15 自己那条");
  const B=device("电脑",repo,{ "catfeed.v3":D({
    upd:900, marks:{"2026-09-15":{a0:1}}, wa:{}, wb:{},
    notes:{"2026-09-15":[{t:"看病",c:"a",x:"复查",at:900}]}
  })}, CFG);
  await wait(300);
  const b=JSON.parse(B.store["catfeed.v3"]);
  ok("手机那天的疫苗拉下来了", (b.notes["2026-09-14"]||[]).length===1);
  ok("电脑自己那条没被冲掉", (b.notes["2026-09-15"]||[]).length===1);
  ok("两天的打勾都在", Object.keys(b.marks).includes("2026-09-14")&&Object.keys(b.marks).includes("2026-09-15"),
     Object.keys(b.marks).sort().join(" "));

  console.log("\n【手机再开】应该把电脑那天也拉回来");
  const A2=device("手机2",repo,A.store,cfg({pulled:["2026"]}));
  await wait(300);
  const a2=JSON.parse(A2.store["catfeed.v3"]);
  ok("两条记事都在手机上了",
     (a2.notes["2026-09-14"]||[]).length===1&&(a2.notes["2026-09-15"]||[]).length===1);
  ok("体重也跟过来了", a2.wa["2026-09-14"]===5.0, JSON.stringify(a2.wa));
